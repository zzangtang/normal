...
def visualize_in_open3d(pc_o3d, selected_point, normal_vector):
    """
    Visualize the point cloud and the selected point's normal vector in Open3D GUI.
    """
    # Create an arrow to represent the normal vector
    arrow = o3d.geometry.TriangleMesh.create_arrow(
        cylinder_radius=0.005, cone_radius=0.0075, cylinder_height=0.05, cone_height=0.02
    )

    # Translate and rotate the arrow to represent the normal vector
    arrow.translate(selected_point)
    direction = normal_vector / np.linalg.norm(normal_vector)
    arrow.rotate(o3d.geometry.get_rotation_matrix_from_axis_angle(direction), center=selected_point)
    arrow.paint_uniform_color([1, 0, 0])  # Red arrow

    # Open Open3D Visualizer
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if state.verts is not None and state.normals_cached is not None:
            ...
            if point_index < len(state.verts):
                ...
                # Launch Open3D Viewer in a separate thread
                threading.Thread(
                    target=visualize_in_open3d,
                    args=(state.pc_o3d, state.selected_point, state.normal_vector),
                    daemon=True,
                ).start()
